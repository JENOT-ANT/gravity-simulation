...
class Object(object):
	...
	def update_position(self, objects):
		update = True
		updated_position = [self.position[0] + self.velocity.x, self.position[1] + self.velocity.y]
		
		for obj in objects:
			if math.dist(updated_position, obj.position) < obj.radius + self.radius and obj.mass > self.mass:#(updated_position[0] > obj.position[0] - obj.radius and updated_position[0] < obj.position[0] + obj.radius) and (updated_position[1] > obj.position[1] - obj.radius and updated_position[1] < obj.position[1] + obj.radius):
				print(obj.color)
				update = False
	...
	def render(self, scale, translation):
		...
		self.RECTANGLE.x = self.position[0] * scale + translation[0] * scale - 10 / 2
		self.RECTANGLE.y = self.position[1] * scale + translation[1] * scale - 10 / 2
		pygame.draw.rect(display, self.color, self.RECTANGLE, 1)#(80, 100, 255)

		pygame.draw.circle(display, self.color, (self.position[0] * scale + translation[0] * scale, self.position[1] * scale + translation[1] * scale), self.radius * scale)


sun = Object(2 * pow(10, 27), [0, 0], 696342, (255, 250, 50))
earth = Object(6 * pow(10, 21), [1.47 * pow(10, 8), 0], 6378, (10, 150, 50))
moon = Object(7.348 * pow(10, 19), [1.47 * pow(10, 8) - 363104, 0], 1737, (50, 50, 50))
...
jupiter = Object(1.898 * pow(10, 24), [7.4052 * pow(10, 8), 0], 70000, (200, 60, 0))
saturn = Object(5.6834 * pow(10, 23), [1.35255 * pow(10, 9), 0], 58232, (215, 160, 100))
...
def main():
	...
	while True:
		...
		if follow == True:
			translation = [-space_craft.position[0] + RESOLUTION[0] / 2 / scale, -space_craft.position[1] + RESOLUTION[1] / 2 / scale]
		...
		for i in range(0, 1):
			...
			sun.update((mercury, wenus, earth, mars, jupiter, saturn), Vector(0, 0))
